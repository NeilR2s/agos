...
class StockApi:
    ...
    def _parse_company_list(
        self, soup: BeautifulSoup, target_tickers: set | None = None
    ) -> list[dict[str, Any]]:
    ...
    def _parse_stock_data(self, html_content: str) -> dict[str, float | None]:
        ...
        price_table = table.find_next("table", class_="view")
        if price_table:
    ...
    async def get_companies(
        self, session: requests.AsyncSession, target_tickers: list[str] | None = None
    ) -> list[dict[str, Any]]:
        ...
        async def fetch_page(page_num: int):
            ...
            r = await self._post(
                session, self.SCRAPE_URL, headers=self._form_headers, data=local_payload
            )
    ...
    async def get_company_details(
        self, session: requests.AsyncSession, cmpy_id: int, sec_id: int, ticker: str, name: str
    ) -> dict[str, Any]:
        ...
        stock_task = self._get(
            session, self.STOCK_DATA_ENDPOINT.format(cmpy_id=cmpy_id, sec_id=sec_id)
        )

        dividend_task = self._post(
            session, self.DIVIDEND_ENDPOINT, data={"cmpy_id": cmpy_id}, headers=self._form_headers
        )

        chart_resp, stock_resp, div_resp = await asyncio.gather(
            chart_task, stock_task, dividend_task
        )
    ...
    async def get_market_summary(self, session: requests.AsyncSession) -> dict[str, Any]:
        ...
        return {"indices": indices, "market": market_summary}
    ...
    async def get_usd_rate(self) -> float | None:
        ...
        try:
            resp = requests.get(
                self.BSP_USD_ENDPOINT, params=params, headers=headers, timeout=self.timeout
            )
