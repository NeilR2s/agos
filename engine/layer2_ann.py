...
try:
    data = pd.read_csv(settings.LAYER1_OUTPUT)
    print(f"Loaded {len(data)} rows from {settings.LAYER1_OUTPUT}")
except FileNotFoundError:
    print(f"Error: Could not find {settings.LAYER1_OUTPUT}. Run layer1_data.py first.")
    exit()
...
print(f"\nTraining Autoencoder for {settings.EPOCHS} epochs...")
...
for epoch in range(settings.EPOCHS):
    ...
    if (epoch + 1) % 10 == 0:
        print(f"  Epoch {epoch+1}/{settings.EPOCHS} — Loss: {total_loss/len(loader):.6f}")
...
threshold = np.percentile(mse, settings.ANOMALY_THRESHOLD_PERCENTILE)
...
data.to_csv(settings.LAYER2_OUTPUT, index=False)
print(f"\nSaved to {settings.LAYER2_OUTPUT}")